...
ALTER SEQUENCE blames_id_seq OWNED BY blames.id;
...
ALTER TABLE ONLY blames ALTER COLUMN id SET DEFAULT nextval('blames_id_seq'::regclass);
...
CREATE INDEX source_maps_env_revision ON source_maps USING btree (environment_id, revision, "from");
...
CREATE RULE occurrences_set_first AS
    ON INSERT TO occurrences DO  UPDATE bugs SET first_occurrence = new.occurred_at
...
CREATE RULE occurrences_set_latest AS
    ON INSERT TO occurrences DO  UPDATE bugs SET latest_occurrence = new.occurred_at
